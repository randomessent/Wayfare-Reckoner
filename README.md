...
**The app** — open `wayfare-app.html` in any browser. No server, no install, no network: the gazetteer, the terrain grid and the coastline are all carried inside the file, which is why it is about ten megabytes.
...
```
wayfare-app.html     the built app — open this
...
which rewrites `wayfare-app.html`. Nothing else is needed.
